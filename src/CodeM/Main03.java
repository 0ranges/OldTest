...
public class Main03 {
    public static void main(String[] args) {
        ...
        double[][] winrate = new double[16][16];
        for(int i = 0;i<16;i++){
            for(int j = 0;j<16;j++)
                winrate[i][j] = input.nextDouble();
        }
        double[] result_last = new double[16];
        Arrays.fill(result_last,1.0);
        double[] result = new double[16];
        for(int i = 1;i<16;i=i*2){
            for(int j = 0;j<16;j++) {
                int i_start = j / i / 2 * 2 + (j / i % 2 == 0 ? 1 : 0);
                i_start *= i;
                for(int k = i_start;k<i_start+i;k++){
                    result[j] += result_last[k]*winrate[j][k];
                }
                result[j] *= result_last[j];
            }
            result_last = Arrays.copyOf(result,16);
            Arrays.fill(result,0.0);
        }
        for(int i = 0;i<16;i++)
            System.out.print(result_last[i]+" ");
    }
}
